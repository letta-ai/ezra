...
async function updateIssue(id: string, updates: { priority?: number; state?: string; assignee?: string; project?: string; label?: string }) {
  ...
  if (updates.assignee) {
    ...
    `;
    const usersData = await graphql(usersQuery);
    const user = usersData.users.nodes.find(
      (u: { id: string; name: string; displayName: string }) => 
        u.name.toLowerCase().includes(updates.assignee!.toLowerCase()) ||
        u.displayName.toLowerCase().includes(updates.assignee!.toLowerCase())
    );
    if (!user) {
      throw new Error(`User not found: ${updates.assignee}`);
    }
  ...
  if (updates.project) {
    ...
    const project = projectsData.projects.nodes.find(
      (p: { id: string; name: string }) => 
        p.name.toLowerCase().includes(updates.project!.toLowerCase())
    );
    if (!project) {
  ...
  try {
    switch (command) {
      ...
      case "update": {
        const id = args[1];
        if (!id) {
          console.error("Usage: update <issue-id> [--priority N] [--state STATE] [--assignee NAME] [--project NAME]");
          process.exit(1);
        }
        const updates: { priority?: number; state?: string; assignee?: string; project?: string } = {};
...
Commands:
  ...
  get <issue-id>
  update <issue-id> [--priority N] [--state STATE] [--assignee NAME] [--project NAME]
